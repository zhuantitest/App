...
// screens/HomeScreen.js（新增）
import dayjs from 'dayjs';
import { getAnalysisOverview, saveBudgetSetting, upsertBucketMap } from '../lib/api';
...
useEffect(() => {
  ...
  (async () => {
    try {
      ...
      const endDate   = dayjs().endOf('month').add(1, 'day').startOf('day').toISOString();
      ...
      // B. 讀一次 overview
      const ov1 = await getAnalysisOverview({ startDate, endDate });
      console.log('[Analysis] before map:', ov1?.buckets);

      // C. 覆寫「食物」為必需，再讀一次（你可改成其他類別）
      await upsertBucketMap({ category: '食物', bucket: 'essential' });
      const ov2 = await getAnalysisOverview({ startDate, endDate });
      console.log('[Analysis] after  map:', ov2?.buckets);
    } catch (err) {
      console.warn('[Analysis SmokeTest] failed:', err?.response?.data || err?.message || err);
    }
  })();
}, []); // 只跑一次
