...
const MY_NAME_KEY = 'myName'; // 與群組頁一致
...
export default function SettingsScreen({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const storedMyName = (await AsyncStorage.getItem(MY_NAME_KEY)) || '';
        if (raw) {
          const { user: u } = JSON.parse(raw);
          setUser(u || null);
          // 有 myName 就優先帶入；否則用 auth.user.name
          setNickname(storedMyName || '');
          setEmailReadonly(u?.email ?? '');
        } else {
          setNickname(storedMyName);
        }
  ...
  const saveNickname = async () => {
    try {
      ...
      Alert.alert('已儲存', '名字已更新，分帳群組會顯示此名字。');
    } catch (e) {
      Alert.alert('錯誤', e?.message ?? '儲存失敗，請稍後再試');
    }
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 24 }}>
      ...
      <View style={styles.card}>
        ...
        <TouchableOpacity style={styles.primaryBtn} onPress={saveNickname} activeOpacity={0.85}>
          <Text style={styles.primaryBtnText}>儲存暱稱</Text>
        </TouchableOpacity>
      </View>
      ...
      <Text style={styles.hint}>提示：此暱稱同時用於分帳群組的你的名字；要修改，請回到本頁更新。</Text>
